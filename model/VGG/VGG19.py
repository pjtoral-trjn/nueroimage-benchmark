...
class VGG19:
  ...
  def get_model(self):
    ...
    ## First layer
    inputs = vgg_block(images, 32, "vgg19_conv_block1", 1)

    ## Second layer
    inputs = vgg_block(images, 64, "vgg19_conv_block2", 2)

    ## Third layer
    inputs = vgg_block(images, 128, "vgg19_conv_block3", 3)

    ## Fourth layer
    inputs = vgg_block(images, 256, "vgg19_conv_block4", 4)

    ## Fifth layer
    inputs = vgg_block(images, 256, "vgg19_conv_block5", 5)
    ...
    outputs = tf.keras.layers.Dense(units=1, name="Cognitive-Assessment-3DVGG19", activation="softmax")(outputs)

    # Define the model
    return tf.keras.Model(images, outputs, name="3DVGG19")